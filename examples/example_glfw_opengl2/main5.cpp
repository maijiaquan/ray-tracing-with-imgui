/*
代码改动：
1.新增随机数函数 random_double()
2.相机封装成类camera
3.入口函数随机采样100次，实现抗锯齿
*/
...
float hit_sphere(const vec3 &center, float radius, const ray &r)
{
    ...
    if (discriminant < 0) //没有实数根
    {
        return -1.0;
    }
    else
    {
        return (-b - sqrt(discriminant)) / (2.0 * a); //返回较小的实数根
    }
}
...
vec3 color(const ray &r, hittable *world)
{
    ...
    {
        return 0.5 * vec3(rec.normal.x() + 1, rec.normal.y() + 1, rec.normal.z() + 1); //可视化RGB通道
    }
    else
    {
